...
def test_ingest_file_path_rejects_outside_allowed_roots_with_clear_message(tmp_path: Path) -> None:
    settings = BackendSettings(data_dir=tmp_path, ingest_allowed_roots=str(tmp_path))
    ...
    with pytest.raises(HTTPException) as exc:
        service.ingest_file_path(user=_admin_user(), department_id="dept-a", file_path="/etc/passwd")
